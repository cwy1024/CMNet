...
class Block(nn.Module):
    def __init__(
        ...
        self.mixer_local = mixer_cls2(dim)
    ...
    def forward(
        ...
        hidden_states_local = rearrange(hidden_states_local, "b (h w) c -> b c h w", h=28,w=28)
        hidden_states_local = F.unfold(hidden_states_local,kernel_size=(2,2),stride=(2,2),padding=0,dilation=1) # b,ckk,h'w'
        hidden_states_local = rearrange(hidden_states_local,'b (c k1 k2) (h1 w1) -> b (h1 w1 k1 k2) c',k1=2,k2=2,h1=14,w1=14)
        hidden_states_local = self.mixer_local(hidden_states_local, inference_params=inference_params)
        hidden_states_local = rearrange(hidden_states_local,'b (h1 w1 k1 k2) c -> b (h1 k1 w1 k2) c',k1=2,k2=2,h1=14,w1=14)

        hidden_states = hidden_states + hidden_states_local
...
def create_b(
    ...
    mixer_cls_local = partial(Mamba, layer_idx=layer_idx, bimamba_type=bimamba_type, if_divide_out=if_divide_out,
                        init_layer_scale=init_layer_scale, **ssm_cfg, **factory_kwargs)
    norm_cls = partial(
...
if __name__ == '__main__':
    ...
    x = torch.rand(24, 4, 224, 224).to(device)
    B, C, H, W = x.shape
    print("输入特征维度：", x.shape)
    x = x.view(B, C, H * W).permute(0, 2, 1)
    print("维度变换：", x.shape)
    mamba = create_b(d_model=C).to(device)
